...
export default async function WorkflowPage({
  ...
  // Ganti Response dengan redirect atau custom error page
  const hasAccess = await workflowRepository.checkAccess(id, session.user.id);
  if (!hasAccess) {
    // Opsi 1: Redirect ke halaman unauthorized
    redirect('/unauthorized');
    
    // Atau Opsi 2: Redirect ke login
    // redirect('/login?redirectTo=' + encodeURIComponent(`/workflow/${id}`));
  }
